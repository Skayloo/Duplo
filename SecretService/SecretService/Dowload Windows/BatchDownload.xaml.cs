            ...
            if (batchUrlValid)
            {
                if (rbFrom1.IsChecked.Value)
                {
                    if ((tbFrom1.Text.Length == 0) || (tbTo1.Text.Length == 0) || (Convert.ToInt32(tbFrom1.Text) > Convert.ToInt32(tbTo1.Text)))
                    {
                        ...
                        this.lblFilesToDownload.Content = "0 файлов загружено";
                        return;
                    }
                }
                ...
                if (downloadUrls.Count == 1)
                    this.lblFilesToDownload.Content = "1 файл загружен";
                else
                    this.lblFilesToDownload.Content = downloadUrls.Count + " файлов загружено";
            }
            else
            {
            ...
            if (downloadUrls.Count > 0)
            {
                try
                {
                    foreach (string url in downloadUrls)
                    {
                        ...
                        {
                            string message = "Такой файл уже существует. Перезаписать?";
                            MessageBoxResult result = Xceed.Wpf.Toolkit.MessageBox.Show(message, "Конфликт имен файлов: " + filePath, MessageBoxButton.YesNo, MessageBoxImage.Warning);
                ...
                Xceed.Wpf.Toolkit.MessageBox.Show("Нет файлов для загрузки!", "Ошибка", MessageBoxButton.OK, MessageBoxImage.Error);
        }

        private void btnBrowse_Click(object sender, RoutedEventArgs e)
        {
            FolderBrowserDialog fbDialog = new FolderBrowserDialog();
            fbDialog.Description = "Выберите путь загрузки";
