    ...
    public class WebDownloadClient : INotifyPropertyChanged
    {
        ...
        public void CheckUrl()
        {
            try
            {
                ...
                using (WebResponse response = webRequest.GetResponse())
                {
                    ...
                    if (this.FileSize <= 0)
                    {
                        Xceed.Wpf.Toolkit.MessageBox.Show("Запрашиваемый файл не существует!", "Ошибка", MessageBoxButton.OK, MessageBoxImage.Error);
                        this.HasError = true;
                    }
                }
            }
            ...
            {
                Xceed.Wpf.Toolkit.MessageBox.Show("Ошибка при получении информации файла. Убедитесь, что URL существует и доступен.",
        ...
        private void CheckBatchUrl()
        {
            ...
            using (WebResponse response = webRequest.GetResponse())
            {
                ...
                if (this.FileSize <= 0)
                {
                    this.StatusString = "Ошибка: Запрашиваемый файл не существует";
                    this.FileSize = 0;
                    this.HasError = true;
                }
        ...
        public void Start()
        {
                ...
                || this.Status == DownloadStatus.Queued || this.HasError)
            {
                ...
                {
                    this.StatusString = "Ошибка: Сервер не поддерживает приостановку загрузки";
                    this.HasError = true;
